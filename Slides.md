---
theme: serif
bg: https://github.com/PabRod/autodiff-slides/blob/main/_meta/_img/escience.png?raw=true
---

<!-- slide bg="https://github.com/PabRod/autodiff-slides/blob/main/_meta/_img/escience-cover.png?raw=true" -->
# JASP-MOD
## An adoption story

By Dr. Pablo Rodríguez-Sánchez

note: this will be invisible in the slide
### Mind map
```mermaid
timeline
    title Talk structure
    Introduction : What is JASP?
			     : Researchers and software
	Users        : The power of GUIs
                 : The power of natural language
    The JASP case: Modules
                 : Docs
				 : Hackathon
    Outro        : We do more than code
```

---
## Before we start
<img src="https://github.com/pabrod/obsidian-slides/blob/main/img/qr.png?raw=true" width="300">

[pabrod.github.io/today](https://pabrod.github.io/today.html)

---

## What is JASP?

--

## Researchers and software

--

![](https://github.com/PabRod/jasp-hackathon-slides/blob/main/img/R-screen.png?raw=true)

--

![](https://github.com/PabRod/jasp-hackathon-slides/blob/main/img/JASP-screen.png?raw=true)

--
![](https://github.com/PabRod/jasp-hackathon-slides/blob/main/img/JASP-world.png?raw=true)

---

## JASP modules

--

![](https://images.unsplash.com/photo-1701339590535-7abecf8d7b3b?q=80&w=1548&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)

--

![](https://github.com/PabRod/jasp-hackathon-slides/blob/main/img/store.png?raw=true)

---

## JASP docs

--

![[https://github.com/PabRod/jasp-story-slides/blob/main/img/docs0.png?raw=true]]

--

![[https://github.com/PabRod/jasp-story-slides/blob/main/img/docs1.png?raw=true]]

--
![[https://github.com/PabRod/jasp-story-slides/blob/main/img/docs3.png?raw=true]]

---

## JASP hackathon(s)



---

#### Summarising
```mermaid
graph LR

Modularity & Documentation & Human["Human contact"] --boosts--> User["User experience"] --improves--> Output["Scientific output"] 
```


--

<!-- slide bg="https://github.com/PabRod/autodiff-slides/blob/main/_meta/_img/escience-cover.png?raw=true" -->
## Research software
### is a form of communication